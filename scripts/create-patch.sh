...
generate_patch()
{
	...
	rm -fr 0001*.patch
	for rev in $revs;do
		# use the dedicated git 1.4.7.1 to avoid diff caused by different git versions
...
# clean the patch and remove the commit id only change to benefit the pdk specific process
cd $top_dir/vendor/intel/google_diffs
python scripts/clean_patch.py
cd $top_dir
